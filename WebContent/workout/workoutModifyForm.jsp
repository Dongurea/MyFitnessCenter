...
<body style="color:#fff;">
 <form action="workoutModifyPro.do" name="workoutModifyForm" method="post" >
...
<div class="container-fluid text-center">    
  <div class="row content">
    ...
    <div class="col-sm-8 text-left"> 
	 ...
	 <button type="button" class="btn btn-default" onclick="location.href='workoutContent.do?workout_name=${workoutDto.workout_name}'">취소</button>
	 </div>
	 <br>
	 <h2 style="color:white;">운동이름</h2><br>
		<input type="text"  style="width:200px; "class="form-control" value="${workoutDto.workout_name}"  readonly="readonly" name="workout_name">	
	  <br>
      <hr>
      <div class="container">
    <div class="row">
      ...
      <textarea name="link" class="form-control" rows="1" cols="3" id="comment">${workoutDto.link}</textarea>
      <hr>
      
      <h2 style="color:white;">운동 1회당 시간</h2><br>
	  <input type="text"  class="form-control" style="width:150px;display:inline" name="sec" value="${workoutDto.sec}"/>&nbsp;초
	...
	<c:set var="workout_part" value="${workoutDto.workout_part}"/>
	<script>
	//<!--
	function workoutpartcheck(){
		
		var workoutpartlist = new Array();
		
		<c:forEach items="${workout_part}" var="workoutpart">
		workoutpartlist.push("${workoutpart}");
		</c:forEach>

		$("input:checkbox[name='workout_part']").each(function(){
			for(var i=0; i<workoutpartlist.length;i++){
				if( workoutpartlist[i]==this.value ){
					this.checked=true;
				}	
			}
		});
